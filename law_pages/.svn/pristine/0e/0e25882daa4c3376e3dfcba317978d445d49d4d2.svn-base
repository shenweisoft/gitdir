...
/*    var iIndex = parseInt(value);
    ...
    var iH = parseInt(szResSel[1]); */
	  var iW = parseInt(3742); 
	    
	    var iH = parseInt(2806); 
...
function SetDeviceRotation(rotation){
    ...
    // alert(rota);
     ...
     //alert(rotation);
     Capture.SetDeviceAngle(szDeviceIndex,rota.toString());
}
...
function Preview(iValue){
	...
	//进入
	return	ShowPreviewBase64(strFileNames[strFileNames.length-1],strFileNames.length);
}
...
function ShowPreviewBase64(strFileName,PreviewWinsowsNo) {
    if(CheckIEVersion() > 8)
	{
		...
		pic.src = 'data:image/jpg;base64,' + strFileName;*/
	}
    return strFileName;
}
...
function FaceVertify()
{
	...
	WriteInfomation("人证匹配結果为"+iResult);
}
...
function LivingBodyDetect()
{
	Capture.StartLivingRecognize(szDeviceIndex, "1", "10000");	
}

function CatchLivingRecognizeResult(iResult)
{
	WriteInfomation("活体检测结果为：" + iResult);
}