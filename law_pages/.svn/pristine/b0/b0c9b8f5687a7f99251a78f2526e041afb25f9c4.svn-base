...
angular.module('sbAdminApp').controller('highMeterCtrl', function($sce,$scope,$log,LawConfig,$timeout,$http,$stateParams,$modalInstance,$modal,DictionaryConfig,AdjustConfig,toaster,Upload,$state,$rootScope,LoginService,CameraService,datamsg) {
	var   num = 0;
    var phi = 0;
	var camidx = 0; 
	var uuid = "";
...
$scope.uploadbase64imgmult=function(){
		...
		//显示进度条
  	$scope.uploadImgProgress = true;

		$(".snapshotimg").each(function(index){
				var num = (index+1) / $(".snapshotimg").length;
				$scope.progressBar = {width: num*100+'%'};
      	$scope.$apply();
