...
{ 
	...
	if(typeof setting.Methods == "object") {
		...
		if(typeof setting.Methods.checkWebServer != "function") {
			setting.Methods.checkWebServer = function(myDevice, applicant) {
					...
					dealDeviceData:function(result){
						...
						if(isLast) {
							...
							if(typeof myDevice == "object") {
								//连接设备，处理返回信息
								setTimeout(function(){
									myDevice.accessDevice(applicant);
								},100);
							}
						}
						else {
				...
				version.accessDevice(applicant);
			}
		}
		//下载驱动默认方法
	...
	if(typeof setting.Cert == "object") {
		...
		ZK.extend(Cert, Device, {
			...
			//接口返回值
			interfaceResult : {
				0:{mean:"成功",message:"识别成功！",type:"success"},
				1:{mean:"端口打开失败",message:"设备未连接，请查看设备情况！",type:"warning"},
				2:{mean:"数据传输超时",message:"设备未连接，请查看设备情况！",type:"error"},
				10:{mean:"没有找到卡",message:"识别失败，请确认身份证是否放置正确！",type:"warning"},
				11:{mean:"读卡操作失败",message:"设备未连接，请查看设备情况！",type:"error"},
				20:{mean:"自检失败",message:"识别失败，请确认身份证是否放置正确！",type:"error"},
				30:{mean:"其他错误",message:"识别失败，请确认身份证是否放置正确！",type:"error"},
				40:{mean:"相片解码失败",message:"识别失败，请确认身份证是否放置正确！",type:"error"},
				100:{mean:"超时",message:"识别失败，请确认身份证是否放置正确！",type:"warning"},
				200:{mean:"GetBase64PhotoData",message:"识别失败，请确认身份证是否放置正确！",type:"error"}
			},
	...
	//处理硬件返回值
	Device.prototype.dealDeviceInfo = function(result, applicant) {
		...
		if("success" === info.type) {
			this.dealDeviceData(result, applicant);
		}
	};

	//html5使用ajax方式访问服务
	Device.prototype.ajaxAccess = function(url, applicant) {
		var result = null;
		var parent = this;
		$.ajax({
			type : "GET",
			...
			success : function(data) {
				...
				parent.dealDeviceInfo(result, applicant);
			},
	...
	//IE8、IE9访问服务
	Device.prototype.xDomainAccess = function(url, applicant) {
		var parent = this;
		...
		if (xDomainRequest) {
			...
			xDomainRequest.onload = function() {
				...
				parent.dealDeviceInfo(obj, applicant);
			};
	...
	//设备通信,得到硬件返回的数据
	Device.prototype.accessDevice = function(applicant) {
		$("#iss").css("width","25%");
		if(browserFlag == "html5"||browserFlag == "simple") {
			this.ajaxAccess(this.url, applicant);
		}
		else if(browserFlag == "simple") {
			this.xDomainAccess(this.url, applicant);
		}
		else {
			if(window.console) {
	...
	Device.prototype.startFun = function(applicant) {
		//创建设备
		var device = new Device();
		var myDevice = device.createDevice("Cert");
		//显示提示信息
		myDevice.setMessage();
		setting.Methods.checkWebServer(myDevice, applicant);
	};
	/*
