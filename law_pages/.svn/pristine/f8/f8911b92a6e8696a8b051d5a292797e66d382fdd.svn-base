angular.module('sbAdminApp').controller('AlgorithmStep5Ctrl', function ($timeout,$scope,$modal, $stateParams, $state, $http, $log,AdjustConfig,AdminConstant, AlgorithmConfig, AlgorithmService, DictionaryConfig, toaster, $filter,$rootScope,$location,LoginService,CalculatorService,AdjustService) {
...
//        	$scope.co.claimantinfochange=claimantinfo;
	    ...
	    $scope.showRespondentFilter = function(algorithmApplyerInfo) {
	        ...
	        $scope.algorithmInfo.jyAlgorithmApplyerInfoVOList.forEach(function(val){
	            if(val.id != $scope.co.claimantinfochange.id){
	                ...
	                if(!obj){
	                	if(val.isOwner!=1&&val.plateNo!=algorithmApplyerInfo.plateNo){
	 ...
	 /*       this.responsibleRate = 0;
	        ...
	        this.riskTypes =angular.copy($scope.riskTypes);
	    ...
	    $scope.generythmDutyInfo = function(val) {
	        ...
	        algorithmDutyInfo.isVehicle = val.isVehicle;
	        //表示为机动车
	        if(algorithmDutyInfo.isVehicle == "1"){
	           // algorithmDutyInfo.riskTypes= true;
	            algorithmDutyInfo.riskTypes[0] = val.riskkinds[0];
	            algorithmDutyInfo.riskTypes[1] = val.riskkinds[1];
	            algorithmDutyInfo.riskTypes[2] = val.riskkinds[2];
	        }
	        ...
	        //医疗费
	        algorithmDutyInfo.medicalFee =val.medicalFee;
	        //死亡伤残
	        algorithmDutyInfo.disabilityFee =val.disabilityFee;
	        //死亡伤残
	        algorithmDutyInfo.propertyLossFee =val.propertyLossFee;
	        //添加集合中
	    ...
	    $scope.addRespondent = function(jyAlgorithmApplyerInfoTab,algorithmApplyerInfo) {
	       ...
	       // algorithmDutyInfo.isVehicle = jyAlgorithmApplyerInfoTab.plateNo ? "1" : "0";
	        ...
	        algorithmDutyInfo.responsibleRate = jyAlgorithmApplyerInfoTab.responsibleRate;
	        algorithmDutyInfo.isVehicle =jyAlgorithmApplyerInfoTab.isVehicle;
	        algorithmDutyInfo.isOwner=jyAlgorithmApplyerInfoTab.isOwner;
	        //this.riskTypeList = angular.copy($scope.riskTypeList);
	        algorithmDutyInfo.riskTypes =angular.copy(jyAlgorithmApplyerInfoTab.riskkinds);
	        algorithmDutyInfo.absDeductibleList=angular.copy(jyAlgorithmApplyerInfoTab.absDeductibleList) ;
	        algorithmDutyInfo.absDeductible=jyAlgorithmApplyerInfoTab.absDeductible;
	        algorithmDutyInfo.thirdPartyRate=jyAlgorithmApplyerInfoTab.thirdPartyRate;
	        algorithmDutyInfo.thirdPartyFranchise=jyAlgorithmApplyerInfoTab.thirdPartyFranchise;
	        algorithmDutyInfo.thirdParty=jyAlgorithmApplyerInfoTab.thirdParty;
	        
	        algorithmDutyInfo.insuranceForceCompany=jyAlgorithmApplyerInfoTab.insuranceForceCompany;
	        algorithmDutyInfo.forceCode=jyAlgorithmApplyerInfoTab.forceCode;
	        
	        algorithmDutyInfo.insuranceBusinessCompany=jyAlgorithmApplyerInfoTab.insuranceBusinessCompany;
	        algorithmDutyInfo.businessCode=jyAlgorithmApplyerInfoTab.businessCode;
	        //.insuranceForceCompany
	        //表示为机动车
	        if(algorithmDutyInfo.isVehicle == "1"){
	            algorithmDutyInfo.riskTypes[0] = jyAlgorithmApplyerInfoTab.riskkinds[0];
	            algorithmDutyInfo.riskTypes[1] = jyAlgorithmApplyerInfoTab.riskkinds[1];
	            algorithmDutyInfo.riskTypes[2] = jyAlgorithmApplyerInfoTab.riskkinds[2];
	        }
...
*/
	    ...
	    $scope.saveMultCaculat=function(){
	     ...
	     $scope.claminfo.forceCode=$scope.claimans[0].forceCode;
	     $scope.claminfo.businessCode=$scope.claimans[0].businessCode;	 
    	 ...
    	 $scope.claminfo.adjust.personName=$scope.claimans[0].personName;
    	...
    	// $scope.claminfo.responsibleRate=$scope.co.responsibleRate; ["0"].compensateRate
    	 $scope.claminfo.responsibleRate=$scope.claimans[0].responsibleRate;  
    	 $scope.claimans[0].jyAlgorithmDutyInfoVOList.forEach(function(val){
	    	...
	    	$scope.savePartClaimData({
				...
				var alinfod=eval('('+$stateParams.alInfo+')');
		    	 $location.url('/dashboard/algorithm/'+alinfod.id+'/algorithmStep4/');
	    ...
	    function recoverall(){
	    	...
	    	var flage=angular.isString(temp);
      	  var clam=null;
      	  if(flage==true){
      		clam=eval('('+temp+')');
      	  }else{
      	 ...
      	 // $scope.userlist.jyAlgorithmApplyerInfoVOList=angular.copy(clam.jyAlgorithmApplyerInfoVOList);
      	  ...
      	  $scope.co.responsibleRate=clam.responsibleRate;
