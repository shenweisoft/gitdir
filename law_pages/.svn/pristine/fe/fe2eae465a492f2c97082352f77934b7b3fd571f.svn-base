  ...
  $scope.policeSubmit = function () {
    if(validateForm()){
      ...
      $scope.adjust.applicantArray.filter(function(v){
        return v.personType==0 && v.delFlag==0;
      }).forEach(function(v){
      ...
      $scope.adjust.applicantArray.filter(function(v){
        ...
        if(adjust.respondent)  adjust.respondent += "," + v.personName;
        else  adjust.respondent = v.personName;
      });
      ...
      adjust.id = $scope.adjust.id;

      $scope.adjust.police.jyAdjustInfoId = $scope.adjust.id;
