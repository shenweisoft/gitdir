...
angular.module('sbAdminApp').controller('noAdjustListCtrl', function ($scope, $log, AdjustService, $state, $modal, DictionaryConfig, $location, $filter, LoginService,toaster,AdjustConfig,$rootScope) {
    ...
    //查询列表Service
    $scope.queryMediationFailureCaseListService = AdjustService.queryMediationFailureCaseListService;
    //查询总数量Service
    $scope.queryMediationFailureCaseCountService = AdjustService.queryMediationFailureCaseCountService;
    //查询是否有单子
    $scope.queryTransferCaseService = AdjustService.queryTransferCaseService;
    //
    //定义查询搜索对象
    function SearchVo(){
        //全局搜索
    ...
    $scope.getDataInfo = function () {
        ...
        //查询列表详细数据
        $scope.queryMediationFailureCaseListService($scope.searchVo).success(function(result) {
        ...
        //查询总条数
        $scope.queryMediationFailureCaseCountService($scope.searchVo).success(function(result) {
            if(result.code == AdjustConfig.commonConStant.SUCCESS){
    ...
    $scope.goCase = function (formCase) {
        //调解中
        var url = $state.href("queryTrackInfo",{serialNo:formCase.serialNo});
    ...
    //转立案
    $scope.updateAdjustState = function(caseInfo , type){
        //查询诉讼表中是否存在流水号的案件，如果不存在则新建案件
        $scope.queryTransferCaseService({
            serialNo:caseInfo.serialNo,
            id:caseInfo.id
        }).success(function(result) {
            if(result.code == AdjustConfig.commonConStant.SUCCESS){
                $log.info(result.result);
                /*if(result.result.isNewCase == '0'){
                    $state.go("dashboard.sue_detail", {serialNo:caseInfo.serialNo, step:1});
                }else{
                    $state.go('dashboard.reading_notes', {"serialNo":caseInfo.serialNo, "name": "reading_notes"});
                }*/
                $state.go("dashboard.sue_detail", {serialNo:caseInfo.serialNo, step:1});
            }else{
                $rootScope.toaster("error", "错误", result.message);
