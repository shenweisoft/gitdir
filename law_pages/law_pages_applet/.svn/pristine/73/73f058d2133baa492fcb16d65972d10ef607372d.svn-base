      ...
      if (res.code) {
        ...
        wx.getUserInfo({
          ...
          success: function (ret) {
            wx.setStorageSync("openUser", ret.userInfo)
...
function proto_getLoginInfo(options) {
  wx.request({
    ...
    success: function (ret) {
      ...
      var openUser = wx.getStorageSync("openUser")
      ret.data.userName = openUser.nickName  || options.userName;
      if (userInfo){
        ret.data.userName = userInfo.text;
      }
      switch (options.type) {
        case 'multi_room': {
          rtcroom.init({
