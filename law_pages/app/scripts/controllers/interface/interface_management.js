 ...
 */
angular.module('sbAdminApp').controller('InterfaceManagementCtrl', function($scope, $modal,$log,toaster,$state,DictionaryConfig,LoginService,LoginConfig,$rootScope) {
  ...
  $scope.getDataInfo = function(pageNo,pageSize){
    ...
    $scope.loginService.queryInterfaceConfigList($scope.interfaceConfigInfo).success(function(result) {
    ...
    //获取总条数
    $scope.loginService.queryCountInterfaceConfig($scope.interfaceConfigInfo).success(function(result) {
      var data = result.result;
      console.log(data);
      if (result.code == LoginConfig.commonConStant.SUCCESS) {
  ...
  $scope.queryInterfaceConfig = function(interfaceConfig){
    var popupModal = $modal.open({//打开弹窗页
      templateUrl:'views/pages/interface_manage/interfacePopup.html',
  ...
  //删除
  $scope.delInterfaceConfig = function(interfaceConfig) {
    if(confirm("您确认删除吗？")){
      var param = [];
      if(interfaceConfig){
      ...
      console.log(param);
      $scope.loginService.deleteInterfaceConfig({
        "id": param
...
angular.module('sbAdminApp').controller('InterfacePopupCtrl', function($scope, $modal,$modalInstance,items,$log,toaster,$state,DictionaryConfig,LoginService,LoginConfig) {
  ...
  $scope.saveInterfaceConfig = LoginService.saveInterfaceConfig;
  ...
  function validateForm(){
    ...
    if(!$scope.interfaceConfigNew.companyName){
      $("[name='name']").focus();
      $scope.titleErrorMessage = $scope.CONSTANT.titleErrorMessage;
      return false;
    }
    if(!$scope.interfaceConfigNew.url){
      $("[name='url']").focus();
  ...
  $scope.save = function(){
    if(validateForm()){
      $scope.saveInterfaceConfig($scope.interfaceConfigNew).success(function(result){
        //请求成功
        if (result.code ==  LoginConfig.commonConStant.SUCCESS) {
          $modalInstance.close({
