...
angular.module('sbAdminApp').controller('LoginCtrl', function($scope, $log, LoginConfig, LoginService, $state,$stateParams) {
    //获取环境配置Service
...
/*  if($scope.userType != null){
    //因为$stateParams的值 页面刷新后就没有了，传过来先放到缓存里
  ...
  $scope.CONSTANT = {
    ...
    "messageForRole":"无当前角色操作权限",
    "messageForLoginLockError":"帐号已被锁定，请联系管理员处理！"
  };
  //登录，提交表单
      ...
      $scope.loginService.login({
        ...
        switch (result.code) {
          ...
          case $scope.loginConstant.loginErrorCode.LOGIN_FAILED_MANY_TIMES:
            $scope.chaptchaClick();
            $scope.errorMessage = $scope.CONSTANT.messageForFailedManyTimes;
            break;
          case $scope.loginConstant.loginErrorCode.LOGIN_ROLE:
            $scope.errorMessage = $scope.CONSTANT.messageForRole;
            break;
          case $scope.loginConstant.loginErrorCode.LOGIN_LOCKED:
            $scope.errorMessage = $scope.CONSTANT.messageForLoginLockError;
            break;
        }
