...
app.controller('step5Ctrl', function(LoginService,$scope, $stateParams, $interpolate, toaster, $state, $location, $timeout, $http, $log, $filter, $modal, AdjustService, AdjustConfig, LawService, DictionaryConfig, CompensateStandardConstant, Upload, IncomeNormConstant, AdminConstant,$rootScope) {
    ...
    //区域
    $scope.adminAdjustRegion = angular.copy(AdminConstant.administrationRegions);
    //后台查询计算器的相关标准
    ...
    //控制树阻止冒泡
    angular.element("#adjustCityNameInput").click(function(e){
        angular.element("#adjustCityNameBox").show();
        stopBubble(e);
    });
    angular.element("#adjustCityNameBox").click(function(e){
        stopBubble(e);
    });
    angular.element("body").click(function(){
        angular.element("#adjustCityNameBox").hide();
    });
    ...
    //选择区域
    $scope.selectAdjustAdmin = function(node, selected, event) {
        var selectedNodes = selected.selected;
        if (selectedNodes.length > 0 && selected.node.type=='default') {
            var selectedRegion = $scope.adminAdjustRegion[selectedNodes[0]];
            if ($scope.adjust.adjustCityName != selectedRegion.fullName) {
                $scope.adjust.adjustCityName = selectedRegion.fullName;
                $scope.adjust.adjustCityCode = selectedRegion.regionCode;
                $scope.adjust.adjustCityNameError = undefined
                angular.element("#adjustCityNameBox").hide();
                $scope.isAdjustShowTree = false;
                //查询调解点
    ...
    $scope.changePoint = function(){
        ...
        var currentPoint = _.find($scope.pointList,{adjustPointCode:$scope.adjust.adjustPointCode});
        $scope.adjust.adjustOrgCode = currentPoint.adjustOrgCode;
        $scope.adjust.adjustOrgName = currentPoint.adjustOrgName;
        $scope.adjust.adjustPointCode = currentPoint.adjustPointCode;
        $scope.adjust.adjustPointName = currentPoint.adjustPointName;
        $scope.adjust.lawOrgName = currentPoint.lawOrgName;
        $scope.adjust.lawOrgId = currentPoint.lawOrgId;
    };
    ...
    $scope.riskTypeChanged = function(respondent, riskType) {
        if (riskType == "0") {
            ...
            if (!respondent.riskTypes['1']){
                ...
                if(!respondent.insuranceBusinessCompany){
                    respondent.insuranceBusinessCompany = ($filter('respondents2Company')($scope.adjust.applicantArray))[0]
                }
    ...
    $scope.feeCheckChanged = function(fee, id) {
        if (fee.isChecked) {
            if (!$scope.adjust.regionName) {
                $rootScope.toaster("error", "错误", "请选择事故发生地!");
                fee.isChecked = false;
                return;
            }
    ...
    $scope.refreshTotal = function() {
        ...
        checkedList.forEach(function(v) {
            ...
            if(v.id == "12" || v.id == "15"){
                ...
                if(v.claimPerUnit && v.claimUnit && v.compensateRate){
    ...
    $scope.adjustResultChanged = function(x) {
        //申请人拼接
        console.log($scope.adjust.applicantArray)
       ...
       /* //费用集合拼接
        ...
        if (x.id == 2) $scope.adjust.adjustResultRemark = "未达成调解，当事双方同意线上开庭";
                ...
                if (i != "undefined") {
                    ...
                    if(payMoney){
                        if(parseFloat(payMoney) < parseFloat(amount)){
                            payMoneyStr = "扣除已支付的"+payMoney+"元，尚余"+(amount-payMoney)+"元，";
                        }
                    }
                    arr.push({
                        name: i,
                ...
                if (v.personType == 1) {
                    ...
                    if(payMoney){
                        if(parseFloat(payMoney) < parseFloat(v.calcIndemnitySum)){
                            payMoneyStr = "扣除申请人"+v.personName+"已垫付的"+payMoney+"元，尚余"+(v.calcIndemnitySum-payMoney)+"元，";
                        }else if(parseFloat(payMoney) > parseFloat(v.calcIndemnitySum)){
                            payMoneyStr = "扣除申请人"+v.personName+"已垫付的"+payMoney+"元，申请人"+jqCompany+"在保险责任范围内按责返还申请人"+ v.personName+(payMoney-v.calcIndemnitySum)+"元，";
                        }
                    }
                    arr.push({
                        "name": v.personName,
            ...
            $scope.adjust.applicantArray.forEach(function(v) {
                if (v.personType == 1 && v.idType == 0) {
                    ...
                    var payMoneyStr = "";
                    if(v.payMoney && !obj){
                        payMoneyStr = "由申请人"+jqCompany+"在保险责任范围内按责返还申请人"+v.personName+"垫付的费用合计"+ v.payMoney+"元，"+payDate;
                        arr.push({
                            "name": v.personName,
                            "amount": v.calcIndemnitySum,
                            "payMoneyStr":payMoneyStr,
        ...
        respondents.forEach(function(v) {
            if ("1" == v.isVehicle) {
                ...
                if(v.riskTypes['0']){
                    ...
                    $scope.noStrongInsurance = (v.personPayDeathSum + v.personPayMedicalSum + v.personPayPropertySum + v.personPaySpiritSum) .toFixed(2);
                }
                //累加被告支付交强险计算总金额
                ...
                if ((v.personPayAmount + claimantExtraMoney) > 0) {
                    ...
                    if(!bxgsFlag){
                        ...
                        $scope.adjust.compensateTable.push({
                            payType: "3",
                            calcFormula: "",
                            calcIndemnitySum: (v.personPayAmount + claimantExtraMoney + parseFloat(noStrongInsurance)).toFixed(2),
